...
from log_utils import log_by_run_time, run_id
...
class EvaRobot(object):
    ...
    def __init__(self):
        ...
        desired_caps['deviceName'] = self.device_tag
        ...
        self.driver = webdriver.Remote('http://localhost:%d/wd/hub' % self.port, desired_caps)
        # 如果发现已安装，先卸载旧的
        ...
        # 建立图片log的目录，根据执行时间
        self.png_log_dir = os.path.join(PNG_DIR_ROOT, run_id)
        self.adb_log_dir = os.path.join(ADB_LOG_DIR_ROOT, run_id)
    ...
    def generate_adb_logs(self, lines=5000):
        ...
        os.system('adb logcat -v time -t ' + str(lines) + ' > ' + adb_log_path)
...
class EvaProcess(object):
    ...
    def log(self, text):
        log_by_run_time(str(self.robot.opt_count) + ' : ' + text)
